  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard title={t('settings.aiPerms')} icon={Shield}>
        ...
        <div className="mt-2 flex items-center justify-between rounded-lg bg-elevated px-3 py-2">
          <span className="text-sm text-ink-soft">{t('chat.autoMemory')}</span>
          <Toggle
            checked={(settings.ai as Record<string, unknown>).autoMemory === true}
            onChange={(v) => update({ ai: { ...(settings.ai || {}), autoMemory: v } })}
            label={t('chat.autoMemory')}
          />
        </div>
      </SectionCard>
